...
func RegisterBackendFactory(factory backend.Factory, configurationSchema *config.ConfigurationSchema) error {
	if backendRegistry[configurationSchema.Name] != nil {
		return fmt.Errorf("The factory already exists %s", configurationSchema.Name)
	}
	backendRegistry[configurationSchema.Name] = &backendConfigurationRegistry{factory, configurationSchema}
	return nil
}
